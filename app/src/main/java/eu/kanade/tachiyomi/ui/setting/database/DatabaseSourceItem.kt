...
class DatabaseSourceItem(val source: Source, val mangaCount: Int) :
    ...
    inner class Holder(view: View, val adapter: FlexibleAdapter<DatabaseSourceItem>) :
        ...
        fun bind(item: DatabaseSourceItem) {
            binding.title.text = if (item.source.id != LocalSource.ID && item.source !is SourceManager.StubSource) {
            ...
            itemView.post {
                when {
                    ...
                    item.source is SourceManager.StubSource -> binding.thumbnail.setImageDrawable(null)
